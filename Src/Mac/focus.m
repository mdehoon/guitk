...
typedef struct TkDisplayFocusInfo {
    NSScreen *screen;		/* Display that this information pertains
				 * to. */
    WidgetObject *focusWinPtr;
...
static DisplayFocusInfo *mainPtrDisplayFocusPtr = NULL;

static DisplayFocusInfo *
FindDisplayFocusInfo(NSScreen* screen)
    	/* screen is the display whose focus information is
				 * needed. */
/*
 ...
 */
{
    ...
    for (displayFocusPtr = mainPtrDisplayFocusPtr;
	    displayFocusPtr != NULL;
	    displayFocusPtr = displayFocusPtr->nextPtr) {
	if (displayFocusPtr->screen == screen) {
fprintf(stderr, "Found old displayFocusPtr\n"); fflush(stderr);
    ...
    displayFocusPtr = PyMem_Malloc(sizeof(DisplayFocusInfo));
    displayFocusPtr->screen = screen;
...
*/
    displayFocusPtr->nextPtr = mainPtrDisplayFocusPtr;
    mainPtrDisplayFocusPtr = displayFocusPtr;
fprintf(stderr, "Returning new displayFocusPtr\n"); fflush(stderr);
    return displayFocusPtr;
}


static WidgetObject* _get_focus_widget(WindowObject* window)
{
    /* window defines the relevant application and display. */
    DisplayFocusInfo *displayFocusPtr;
    NSScreen* screen;
    ...
    displayFocusPtr = FindDisplayFocusInfo(screen);
    return displayFocusPtr->focusWinPtr;
}

void
_set_focus_widget(WidgetObject* widget, bool force)
{
    WidgetView* view = widget->view;
    NSWindow* window = [view window];
    [window makeFirstResponder: view];
    if ([NSView.focusView window] != window) {
    }

#ifdef FINISHED
    DisplayFocusInfo *displayFocusPtr;
    NSScreen* screen = [[widget->view window] screen];
    WidgetObject *topLevelPtr;
...
Application_focus(PyObject* unused, PyObject* args, PyObject* keywords)
{
    ...
    if (widget == NULL) {
         ...
         */
	PyObject *object = (PyObject *)_get_focus_widget(NULL);
    ...
    _set_focus_widget(widget, false);
#ifdef FINISHE
	...
	newPtr = _get_focus_widget(newPtr);
	if (newPtr != NULL) {
				 ...
				 * event. */
{
	...
	_set_focus_widget(winPtr, eventPtr->xfocus.detail);
	return 0;
    }
    ...
    XEvent *eventPtr)		/* Information about event. */
{
	...
	displayFocusPtr->focusOnMapPtr = NULL;
	_set_focus_widget(winPtr, displayFocusPtr->forceFocus);
    }
}
