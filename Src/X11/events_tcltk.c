...
static void
MyRecursive(Widget widget, XtWidgetProc proc)
{
    register Cardinal i;
    ...
    if (XtIsComposite(widget)) {
        ...
        for (i = 0; i < cwp->num_children; i++) {
            MyRecursive(cwp->children[i], proc);
        }
    }
    ...
    if (XtIsWidget(widget)) {
        for (i = 0; i < widget->core.num_popups; i++) {
            MyRecursive(widget->core.popup_list[i], proc);
        }
    }
...
static void
MyPhase2Callbacks(Widget widget)
{
...
static Boolean
MyIsDescendant(Widget widget, Widget root)
{
...
static void
MyPhase2Destroy(register Widget widget)
{
...
static void
MyXtPhase2Destroy(Widget widget)
{
    ...
    MyRecursive(widget, MyPhase2Callbacks);
    if (app->destroy_count > starting_count) {
        ...
        while (i < app->destroy_count) {
            ...
            if (MyIsDescendant(dr->widget, widget)) {
    ...
    MyRecursive(widget, MyPhase2Destroy);
...
static void
_MyXtDoPhase2Destroy(XtAppContext app, int dispatch_level)
{
...
MyXtDispatchEvent(XEvent *event)
{
    ...
    if (app->destroy_count > starting_count)
        _MyXtDoPhase2Destroy(app, dispatch_level);
